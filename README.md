...
- [x] **Vélib'-Station am Hotel verifiziert, korrigiert (2026-09-12)**: statt "Pyrénées - Avron" (73 Rue des Pyrénées) jetzt "Cours de Vincennes - Soult" (Cours de Vincennes/Ecke Boulevard Soult, 75012, vom Nutzer per Google-Maps-Link vorgegeben) — laut offiziellem GBFS-Live-Feed (velib-metropole-opendata.smovengo.cloud) verifiziert: stationCode 12020, 45 Docks, `is_installed`/`is_renting` = 1, beim Test 5 Räder frei (2 mechanisch, 3 E-Bike). Bleibt trotzdem sinnvoll, die Radverfügbarkeit kurz vor Abfahrt in der App zu checken — die schwankt stündlich.
...
## Themen-Badges entfernt (2026-09-12, selber Tag)

Auf Wunsch ("macht wenig Sinn") die farbigen Themen-Badges (Klimaschutz/Klimaanpassung/Nachhaltiges Wirtschaften) auf jeder Station entfernt — alle 12 `<span class="tag tag-…">`-Elemente in `index.html` gelöscht (10 Stationen + Ausklang + La Caverne). Die zugehörigen, jetzt ungenutzten CSS-Regeln (`.tag`, `.tag-schutz`, `.tag-anpassung`, `.tag-wirtschaft`) ebenfalls entfernt, statt als toten Code stehen zu lassen. Die drei Farben selbst (`--green`, `--blue`, `--orange`) bleiben im Stylesheet, da sie weiterhin für den Header-Verlauf und den Rahmen der orangen Kritik-Box gebraucht werden.

Folgeänderung: Struktur-Abschnitt oben präzisiert (Themenfarben nicht mehr als Pro-Station-Badge, nur noch in anderen Elementen).

**Nachtrag, selben Tag:** Auf Nachfrage geprüft und `karte-uebersicht.html` ebenfalls angepasst — die Marker-Popups zeigten dort noch dieselben Themen-Badges, und die Karte hatte zusätzlich eine eigene Farblegende (grün/blau/orange nach Thema) für die nummerierten Marker. Beides entfernt: Marker sind jetzt einheitlich dunkel (`#1a1a1a`, dieselbe Farbe wie die Stationsnummern-Kreise in `index.html`), keine Legende mehr nötig. Zugehörige, jetzt ungenutzte CSS-Regeln (`.legend`, `.popup-tag`) und die nicht mehr referenzierte CSS-Variable `--orange` ebenfalls entfernt. Die Stationsreihenfolge/Koordinaten in der Karte waren bereits korrekt (spiegelten schon die aktuelle Nummerierung 1 Tour de la Biodiversité … 10 La Recyclerie) und mussten nicht angepasst werden.

## Vélib'-Station am Hotel ersetzt (2026-09-12, selber Tag)

Auf Wunsch die im Start-Abschnitt empfohlene Vélib'-Station gewechselt: statt "Pyrénées - Avron" (73 Rue des Pyrénées, 75020) jetzt **"Cours de Vincennes - Soult"** (Cours de Vincennes/Ecke Boulevard Soult, 75012) — Adresse aus dem vom Nutzer geteilten Google-Maps-Link aufgelöst (Pin landete auf "Vélib' Métropole" ohne Klartext-Adresse, per Koordinatenabgleich mit dem offiziellen GBFS-Stationsverzeichnis eindeutig als stationCode 12020 identifiziert).

Live-Status per GBFS verifiziert (velib-metropole-opendata.smovengo.cloud): `is_installed`/`is_renting`/`is_returning` = 1, Kapazität 45 Docks, beim Test 5 Räder frei (2 mechanisch, 3 E-Bike) und 38 freie Docks.

Folgeänderungen:
- `index.html`, Start-Abschnitt: Stationsname, Adresse und Kartenlink (jetzt auf Koordinaten statt Strassenadresse, da "Cours de Vincennes" keine Hausnummer hat) aktualisiert.
- Checkliste oben aktualisiert und als Korrektur markiert statt den alten, jetzt falschen Verifizierungs-Eintrag stehen zu lassen.

## Quellen

Recherchiert Ende Juli/Anfang August 2026, siehe Chatverlauf für Einzel-Quellen (Ville de Paris, FRANCE 24, Brut, Le Nouvel Obs, INRAE-Kritik an der Miyawaki-Methode via tela-botanica.org/enlargeyourparis.fr, Mairie du 13e zur Petite Ceinture, Bloomberg CityLab und reporterre.net zur Voie Georges Pompidou). Ergänzt am 29./30.08.2026 um Republik, "Au revoir, Paris" (Kolly, 2026), paris.fr/Mairie du 11e/Mairie du 18e zum "rues aux écoles"-Programm, education.gouv.fr zu den Schulen an der Rue Ferdinand-Flocon, sowie paris-blog.org ("Die Petite Ceinture", Teil 1+2, 2020) und die offizielle paris.fr-Seite zu Square Charles Péguy für die neue Station 1 und den Ausbau der Petite-Ceinture-Station. Für die Konnektivitätsfrage (Porte de Choisy) zusätzlich mairie13.paris.fr und petiteceinture.org/ASPCRF (Statusbericht Dezember 2024). Für die Velo-Infrastruktur-Prüfung (30.08.2026) zusätzlich: rerv.fr und Wikipedia zum Réseau Vélo Île-de-France, mdb-idf.org (Mieux se Déplacer à Bicyclette, u. a. zur Avenue des Gobelins und den Boulevards des Maréchaux), mairie11.paris.fr/mairie12.paris.fr zu Boulevard Voltaire und Rue du Faubourg Saint-Antoine, parisenselle.fr zur "vélorue" Rue de la Roquette, cdn.paris.fr (Bauphasen-PDF Boulevard Saint-Michel), mairie10.paris.fr zu Boulevard Magenta, sowie parisjetaime.com zu den vier Pariser Rad-Hauptachsen. Für die École-Damesme-Korrektur (12.09.2026) zusätzlich: paris.fr/pages/57-nouvelles-rues-aux-ecoles-dans-paris-8197 (vom Nutzer nachgereicht, per Abruf verifiziert). Für die Google-Maps-Wegpunkt-Limite (12.09.2026) zusätzlich: developers.google.com/maps/documentation/urls (offizielle Doku: 9 Wegpunkte am Rechner, 3 auf dem Handy für das `maps/dir/?api=1`-URL-Schema). Für die Vélib'-Stationskorrektur (12.09.2026) zusätzlich: velib-metropole-opendata.smovengo.cloud (offizieller GBFS-Feed, station_information.json und station_status.json, per Abruf verifiziert).
